...
(defun fontify-loop (lst)
  ...
  (let ((1st (pop lst)))
    (editor::font-lock-apply-highlight (first 1st) (second 1st) 'macro-face))
  (dolist (form lst)
...
(defun fontify-declaration-list (lst)
  ;; FIXME: buggy?
  (let ((1st (pop lst)))
    (editor::font-lock-apply-highlight (first 1st) (second 1st) 'declaration-face))
  (dolist (form lst)
    (let* ((start (first form))
