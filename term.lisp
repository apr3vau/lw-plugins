            ...
            (prog ((pending (cdr pending-sequence))
              ...
              ed (case (or param1 0)
                   ...
                   (1 (editor:with-point ((start pt))
                        (editor:buffer-end start)
                        (editor:line-offset start (- (1- height)) 0)
                        (let ((str (string-append (make-string (- height 1 (count-lines-after pt)) :initial-element #\Newline)
                                                  (make-string (editor:point-column pt) :initial-element #\Space))))
                          (editor:delete-between-points start pt)
                          (editor:insert-string pt str)
                          (editor:character-offset pt (length str)))))
                   (t (editor:with-point ((start pt)
                                          (end pt))
                        ...
                        (let* ((lines-after (count-lines-after pt))
                          ...
                          (editor:insert-string pt str)
                          (editor:character-offset pt str)
                          (editor:insert-string pt (make-string lines-after :initial-element #\Newline))))))
              (go finish)
              el (case (or param1 0)
                        ...
                        (let ((col (editor:point-column pt)))
                          ...
                          (editor::big-replace-string
                           ...
                           (editor::make-buffer-string
                            :%string (make-string (- (editor:point-position end) (editor:point-position start))
