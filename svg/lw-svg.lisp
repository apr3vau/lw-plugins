...
element."
  ...
  (let ((tag (plump-dom:tag-name node))
    ...
    (labels (;; Helper functions & Getter of presentation attributes
             ...
             (get-font ()
               (let* ((fdesc (gp:font-description (gp:graphics-port-font port)))
                      (family (if-let (family (get-attr "font-family"))
                                  (let ((subs (split-sequence '(#\, #\Space #\") family :coalesce-separators t)))
                                    (dolist (sub subs)
                                      (string-case (sub)
                                        ("serif"
                                        ...
                                        ("monospace"
                                         ...
                                         (return))
                                        (t (handler-case
                                               (when (gp:find-matching-fonts port (gp:make-font-description :family sub))
                                                 (setq family sub)
                                                 (return))
                                             (error (e) nil)))))
                                    (if (stringp family) family
                                      (gp:font-description-attribute-value fdesc :family)))
                                (gp:font-description-attribute-value fdesc :family)))
                      (size (if-let (size (get-attr "font-size"))
                                (round (svg-parse-length size :width))
                              (gp:font-description-attribute-value fdesc :size)))
                      (slant (if-let (style (get-attr "font-style"))
                                 (string-case (style)
                                   ("normal" :roman)
                                   ("italic" :italic)
                                   ("oblique" :oblique))
                               (gp:font-description-attribute-value fdesc :slant)))
                      (weight (if-let (weight (get-attr "font-variant"))
                                  (if (digit-char-p (char weight 0))
                                    ...
                                    (string-case (weight)
                                      ("normal" :normal)
                                      ("bold" :bold)
                                      (t (gp:font-description-attribute-value fdesc :weight))))
                                (gp:font-description-attribute-value fdesc :weight))))
                 (gp:find-best-font port (gp:make-font-description :family family :size size :slant slant :weight weight))))
             ...
             (run-draw-path (trans-origin-x trans-origin-y path)
               (let* ((transform (multiply-transforms-for-drawing trans-origin-x trans-origin-y))
                      (fill (get-fill))
                      (stroke (get-stroke))
                           ...
                           (do ((x left (1+ x)))
                             ...
                             (do ((y top (1+ y)))
                               ...
                               (multiple-value-bind (origin-x origin-y)
                                   (gp:untransform-point grad-trans x y)
                                 (gp:draw-point port x y :foreground (svg-gradient-color origin-x origin-y grad))))))
                         (when stroke
             ...
             (collapse-whitespace-around-children (node)
               (let ((children (plump:children node))
                 ...
                 (unless (equalp space "preserve")
                   (macrolet ((trim (text) `(string-left-trim '(#\Space #\Tab #\Return #\Newline) ,text)))
                     (let ((start (aref children 0))
                           (end (aref children (1- (length children)))))
                       (when (plump-dom:text-node-p start)
                         (setf (plump:text start) (trim (plump:text start))))
                       (when (plump-dom:text-node-p end)
                         (setf (plump:text end) (trim (plump:text end)))))))))
             (draw-a-character (char font transform)
                 ...
                 (case (gethash "writing-mode" container-attributes)
                   ...
                   (:tb
                    (setf (gethash "text-total-height" container-attributes)
                          (+ (gethash "text-total-height" container-attributes 0)
                             char-height))))
                 (let ((transform (gp:copy-transform transform)))
                   (apply-rotation-around-point transform r1 x y)
                   (lambda (port)
                     (gp:draw-character port char x (+ y char-ascent)
                                        :font font :transform transform :foreground fill)))))
             (draw-a-text-node (text-node)
               (let* ((transform (multiply-transforms-for-drawing
                                  (gethash "text-start-x" container-attributes)
                                  (gethash "text-start-y" container-attributes)))
                      (font (get-font))
                      (space (gethash "text-space" container-attributes)))
                 (unless (equalp space "preserve")
                 ...
                 (when-let (funcs (loop for char across (plump:text text-node)
                                        for func = (draw-a-character char font transform)
      ...
      (string-case (tag)
           ...
           (when-let (funcs (loop for child across children
                                  for func = (cond ((plump-dom:textual-node-p child)
                                                   ...
                                                   ((plump-dom:element-p child)
                                                    (create-renderer port child root-node new-table)))
                                  when func collect func))
             (lambda (port)
               (dolist (func funcs)
                 (funcall func port))))))
        ("tspan"
         ...
         (when-let (new (coerce (css-parse-all-angels-from-string (gethash "rotate" new-attrs)) 'list))
           (setf (gethash "text-rotate" container-attributes) (append new (gethash "text-rotate" container-attributes))))
         ...
         (when-let (funcs (loop for child across (plump-dom:children node)
                                for func = (cond ((plump-dom:textual-node-p child)
                                                  (draw-a-text-node child))
                                                 ((plump-dom:element-p child)
                                                  (create-renderer port child root-node container-attributes)))
                                when func collect func))
           (lambda (port)
             (dolist (func funcs)
               (funcall func port)))))
                 ...
                 ("svg"
                  (capi:with-geometry port
                    (let ((viewbox (if-let (val (gethash "viewBox" new-attrs))
                                     ...
                                     (list 0 0 capi:%width% capi:%height%))))
